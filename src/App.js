  ...
  return (
    <>
      ...
      <h1 className="text-center my-3" style={{ color: "white" }}>
        Tic <span className="span">Tac</span> Toe
      </h1>
      <StatusMessage current={current} winner={winner} />
      <div className="app">
        <Board
          state={current.state}
          handleClick={updateCell}
        ...
        <History className="history" history={history} moveTo={moveTo} currentMove={currentMove} />
      </div>
      <div className="footer" /*style = { {top: 'window.innerHeight'} }*/></div>
    </>
  );
}
